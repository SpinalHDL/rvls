...
class RvlsBackend(workspace : File = new File(".")) extends TraceBackend{
  ...
  override def commit(hartId: Int, pc: Long): Unit = if(!Frontend.commit(handle, hartId, pc)) throw new Exception(Frontend.getLastErrorMessage(handle))
  override def trap(hartId: Int, interrupt: Boolean, code: Int): Unit = if(!Frontend.trap(handle, hartId, interrupt, code)) throw new Exception(Frontend.getLastErrorMessage(handle))
